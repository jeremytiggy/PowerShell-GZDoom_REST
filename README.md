...
4a. To get started the fastest, use the directory and configuration file as is. The 'gzdoom_portable.ini' contains an autoload instruction for the 'addons' directory. Place the PK3 there to automatically load on startup.
4b. If you're using a loader like DoomRunner, add the PK3 to the load order.
4c. You can successfully overwrite any GZDoom v4.14.2 installation by just replacing the 'gzdoom.exe' file with the one from the archive, and it will add Pipe functionality. Make sure to add the ExternalPipeSettingsMenu.pk3 from the addons directory to that load order.
